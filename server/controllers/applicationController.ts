...
const applicationController: ApplicationController = {
	...
	getApplications: async (req: any, res, next) => {
		try {
			...
			console.log('id', id);
			const queryString = `
      SELECT a.id AS app_id, a.company, a.location, a.position, a.application_url, a.notes, u.userID, a.modified_at, s.status_name, s.status_rank, s.created_at AS status_created_at, s.modified_at AS status_modified_at, s.id AS status_id
	...
	addApplication: (req: any, res, next) => {
		...
		const addApplicationQuery =
			'INSERT INTO applications (company, location, position, application_url, notes, user_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id';
		...
		applicationModel.query(addApplicationQuery, params, (err, app) => {
			if (err)
				...
				const addStatusQuery =
					'INSERT INTO status (status_name, status_rank, app_id) VALUES ($1, $2, $3)';
		...
		applicationModel.query(deleteStatusQuery, undefined, (err) => {
			if (err)
				return next({
					...
					message: {
						error:
							'Error in applicationController: deleteApplication (delete status)',
					},
					status: 500,
				});
			applicationModel.query(deleteOfferQuery, undefined, (err) => {
				if (err)
					return next({
						...
						message: {
							error:
								'Error in applicationController: deleteApplication (delete offer)',
						},
						status: 500,
					});
				else {
					applicationModel.query(deleteApplicationQuery, params, (err) => {
						if (err)
							return next({
								...
								message: {
									error:
										'Error in applicationController: deleteApplication (delete application)',
								},
								status: 500,
							});
						else {
	...
	updateApplication: async (req: any, res, next) => {
		...
		for (let i = 0; i < updateAppFields.length; i++) {
			if (i !== updateAppFields.length - 1)
				updateAppInfoQuery += `${updateAppFields[i]} = '${updateAppValues[i]}', `;
			else {
		...
		for (let i = 0; i < updateStatusFields.length; i++) {
			if (i !== updateStatusFields.length - 1)
				updateStatusInfoQuery += `${updateStatusFields[i]} = '${updateStatusValues[i]}', `;
			else {
		...
		applicationModel.query(updateAppInfoQuery, params, (err, app) => {
			if (err)
				return next({
					log: `applicationController: Error: ${err}`,
					message: {
						error: 'Error in applicationController: updateApplication',
					},
					status: 500,
				});
			else {
